...
pipeline {
  agent {
    kubernetes {
      cloud cloudName(DEPLOY_ENV)
      yaml """
        apiVersion: v1
        kind: Pod
        metadata:
          labels:
            project: idgenerator_project_builder_pod
        spec:
          volumes:
          - name: docker-sock
            hostPath:
          ...
          containers:
          - name: centos
            image: docker.komus.net/centos:7
            command:
            - cat
            tty: true
            privileged: true
          ...
          - name: docker
            image: docker.komus.net/docker:20.10-git
            command:
            - sleep
            args:
            - 1h
  ...
  parameters {
    ...
    choice(name: 'DEPLOY_ENV',
      choices: ['id-dev','imkqa-shared','imktklt','lt','r291lt','prod'],
  ...
  environment{
    ...
    HELM_DST="https://nexus.komus.net/repository/common/helm/3.7.1/linux/amd64/helm-v3.7.1-linux-amd64.tar.gz"
    HELM_REPO_URL="https://nexus.komus.net/repository/helm-internal/"
    KUBECTL_DST="https://nexus.komus.net/repository/common/kubectl/v1.22.0/linux/amd64/kubectl"
  ...
  stages {
      ...
      steps {
        dir(BUILD_DIR){
          script {
            ...
            // if (APP_VERSION.endsWith("-SNAPSHOT") && params.DEPLOY_ENV == 'prod'){
            //   println "${BUILD_NUMBER}-WRONG: DEPLOY ENV IS ${DEPLOY_ENV} AND APP_VERSION ENDS WITH ${APP_VERSION} "
            //   currentBuild.displayName = "##${BUILD_NUMBER}-WRONG: ${DEPLOY_ENV} AND ${APP_VERSION} NOT ALLOWED. GIT BRANCH: ${BRANCH_NAME}"
            //   currentBuild.result = 'ABORTED'
            //   error("${BUILD_NUMBER}-WRONG: DEPLOY ENV IS ${DEPLOY_ENV} AND APP_VERSION ENDS WITH ${APP_VERSION}. GIT BRANCH: ${BRANCH_NAME} ")
            // }
    ...
    stage('Prepare image to run helm charts') {
      steps {
        container('centos') {
          sh '''
            yum install curl -y
            ...
            helm repo add komus ${HELM_REPO_URL}
          '''
        }
      }
    }
      ...
      steps {
        container('centos'){
          withVault([configuration: vaultConfiguration, vaultSecrets: secretsFromVault]){
            script {
              env.HELM_DEPLOY_ARGS = "--install ${APP_NAME}-${DEPLOY_ENV} komus/${HELM_CHART_NAME}"
              env.HELM_DEPLOY_ARGS += " --version ${HELM_CHART_VERSION}"
            ...
            sh '''
             echo "###DEPLOY IN K8S###"
             helm upgrade --cleanup-on-fail ${HELM_DEPLOY_ARGS} --wait
            '''
          }
        }
      }
